 ...
 * @author asus
 */
public class Interface_choisir_jobeur_renovationController implements Initializable {
    ...
    @FXML
    private TableView<Jobeur> Table_Jobeurre;
    @FXML
    void RechercheJobeur(ActionEvent event) {
     ...
     @FXML
    void Onkeyjobeur_reno(KeyEvent event) {
     ...
     public void afficherTab_service()
    {
     ...
     public static Jobeur jre;
     
     Gestion_tableau_jobeur gt = new Gestion_tableau_jobeur();
    ArrayList jobeur= (ArrayList) gt.afficherJobeur_Renovation();
    ...
    public void initialize(URL url, ResourceBundle rb) {
        afficherTab_service();
        Table_Jobeurre.setOnMouseClicked((MouseEvent event) -> {
            if (event.getButton().equals(MouseButton.PRIMARY) && event.getClickCount() == 2){
         Jobeur A=new Jobeur();
          A = (Jobeur) Table_Jobeurre.getItems().get(Table_Jobeurre.getSelectionModel().getSelectedIndex());
         jre=A;
          Parent root = null;
                System.out.println(A);
                try {
                    root = FXMLLoader.load(getClass().getResource("/fxml/affiche_profilJobeur_renovation.fxml"));
