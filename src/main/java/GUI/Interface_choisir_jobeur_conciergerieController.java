 ...
 * @author asus
 */
public class Interface_choisir_jobeur_conciergerieController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        afficherTab_service();
        Table_Jobeurc.setOnMouseClicked((MouseEvent event) -> {
            if (event.getButton().equals(MouseButton.PRIMARY) && event.getClickCount() == 2){
         Jobeur A=new Jobeur();
          A = (Jobeur) Table_Jobeurc.getItems().get(Table_Jobeurc.getSelectionModel().getSelectedIndex());
         jco=A;
          Parent root = null;
                System.out.println(A);
                try {
                    root = FXMLLoader.load(getClass().getResource("/fxml/affiche_profilJobeur_conciergerie.fxml"));
