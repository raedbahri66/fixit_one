 ...
 * @author asus
 */
public class Interface_choisir_jobeur_jardinageController implements Initializable {
    ...
    @FXML
    private TableView<Jobeur> Table_Jobeurj;
    @FXML
    ...
    @FXML
    void Onkeyjobeur_jardi(KeyEvent event) {
     ...
     public void afficherTab_service()
    {
     ...
     public static Jobeur jj;
     
     Gestion_tableau_jobeur gt = new Gestion_tableau_jobeur();
    ArrayList jobeur= (ArrayList) gt.afficherJobeur_Jardinage();
    ...
    public void initialize(URL url, ResourceBundle rb) {
        afficherTab_service();
        Table_Jobeurj.setOnMouseClicked((MouseEvent event) -> {
            if (event.getButton().equals(MouseButton.PRIMARY) && event.getClickCount() == 2){
         Jobeur A=new Jobeur();
          A = (Jobeur) Table_Jobeurj.getItems().get(Table_Jobeurj.getSelectionModel().getSelectedIndex());
         jj=A;
          Parent root = null;
                System.out.println(A);
                try {
                    root = FXMLLoader.load(getClass().getResource("/fxml/affiche_profilJobeur_jardinage.fxml"));
